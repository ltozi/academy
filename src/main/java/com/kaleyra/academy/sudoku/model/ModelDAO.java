...
/**
 * Oggetto di accesso ai dati, raccoglie le funzionalità per esportare e
 * importare i modelli di gioco su file di testo
 *
...
public class ModelDAO {
	...
	private static Logger logger = Logger.getLogger("com.kaleyra.academy.sudoku.model.ModelDAO");
	/**
	...
	/**
	 * Carica un gioco in memoria e restituisce un modello che contiene le relative
	 * informazioni
	 *
	...
	public GameModel load(String filename) throws SudokuException {
		...
		int[][] userCells = new int[GameModel.ROWS][GameModel.COLS];
		...
		try {
			...
			throw new SudokuException("Impossibile caricare il gioco " + filename, e);
		}
	}
		...
		try {
			...
			for (byte row = 0; row < GameModel.ROWS; row++) {
				for (byte col = 0; col < GameModel.COLS; col++) {
					...
					String flag = model.isPredefined(row, col) ? PREDEFINED_TYPE : USER_TYPE;
